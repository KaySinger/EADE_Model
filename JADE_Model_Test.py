...
# 设置变量边界
bounds = np.array([(1.0, 1.0)] + [(0, 2.0)] * 39 + [(0, 0.5)] * 39)
...
# 运行差分进化算法
best_solution, best_fitness, fitness_history = jade(objective_global, bounds=bounds, pop_size=400, max_gen=5000, p=0.1, tol=1e-6)
print("全局优化得到的系数k:", {f'k{i}': c for i, c in enumerate(best_solution[:40], start=0)})
print("全局优化得到的系数k_inv:", {f'k{i}_inv': c for i, c in enumerate(best_solution[40:], start=1)})
