...
def crossover(X, V, CR):
    ...
    for i in range(NP):
        j_rand = np.random.randint(0, D)  # 确保至少有一个维度来自变异向量
        for j in range(D):
...
# 定义梯度优化函数
def gradient_optimization(func, x, bounds):
    result = minimize(
        func,  # 目标函数
        x,  # 初始点
...
def DPADE(func, bounds, pop_size=None, max_gen=None, hist_size=100, tol=1e-6):
    """
    改进的 SHADE 算法，结合孙理胜论文中的策略和控制参数自适应机制。
    """
    dim = len(bounds)
    archive = []
    H = hist_size
    ...
    CR_min, CR_max = 0.5, 0.8  # 交叉因子 CR 的范围
    st_max = 20  # 停滞代数的上限
    ...
    for gen in range(max_gen):
        ...
        # 动态调整参数 p
        p = p_max - (gen / max_gen) * (p_max - p_min)

        # 种群分类
        fitness_sorted_indices = np.argsort(fitness)
        NPG_size = max(4, int(pop_size * (1 - gen / max_gen)))  # 优势种群逐渐减小到4
        ...
        for i in range(pop_size):
            ...
            # 根据迭代阶段选择变异策略
            if gen < max_gen / 4:  # 迭代早期
                mutant = rand1bin(pop, F, i)
            else:  # 迭代中期和后期
        ...
        # 每 100 次迭代对前 20% 的优秀个体进行一次梯度优化（仅在前80%的迭代过程中进行）
        if (gen + 1) % 100 == 0 and gen < 0.8 * max_gen:
            elite_size = int(0.2 * pop_size)  # 前 20% 的优秀个体
            elite_indices = np.argsort(fitness)[:elite_size]

            # 并行化梯度优化
            results = Parallel(n_jobs=-1)(
                delayed(gradient_optimization)(func, pop[idx], bounds) for idx in elite_indices)

            for idx, (optimized_x, optimized_fitness) in zip(elite_indices, results):
                if optimized_fitness < fitness[idx]:  # 如果优化结果更好，则更新个体
                    pop[idx] = optimized_x
                    fitness[idx] = optimized_fitness
...
# 设置变量边界
bounds = np.array([(2.0, 2.0)] + [(0.001, 10.0)] * 9)
...
# 运行差分进化算法
best_solution, best_fitness, fitness_history = DPADE(objective_global, bounds=bounds, pop_size=100, max_gen=1000, hist_size=100, tol=1e-6)
